...
class App extends React.Component {
  state = {
      ...
      fees:"",
      language:"",
      doctorslist:doctorsList // Initializing the state with the list of doctors
  }

  // Functions to handle changes in filters
  onchangeLocation = (activevalue) => {
      this.setState({ location: activevalue });
  };
  onchangeSearch = (activeSearch) => {
      this.setState({search:activeSearch})
  }
  onchangeExpertise = (activeExpertise) => {
      this.setState({expertise:activeExpertise})
  }
  onchangeGender = (activeGender) => {
      this.setState({gender:activeGender})
  }
  onchangeFees = (activeFees) => {
      this.setState({fees:activeFees})
  }
  onchangeLanguage = (activeLanguage) => {
      this.setState({language:activeLanguage})
  }

  render() {
      // Destructuring state variables
      const {location,search,expertise,gender,fees,language,doctorslist} = this.state
      return (
          ...
          <ReactContext.Provider
              value={{
                  ...
                  onchangeExpertise:this.onchangeExpertise,
          ...
          >
              <BrowserRouter>
                  ...
                  <Routes>
                      ...
                      <Route exact path="/doctors" element={<Finddoctors />} />
                  </Routes>
              </BrowserRouter>
          </ReactContext.Provider>
      );
